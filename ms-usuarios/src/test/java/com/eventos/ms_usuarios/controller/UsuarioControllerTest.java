...
class UsuarioControllerTest {
    ...
    void crearUsuario_ok() {
        ...
        Mockito.when(usuarioService.crearUsuario(Mockito.any()))
                .thenReturn(new com.eventos.ms_usuarios.dto.UsuarioDto(
                        1L,
                        dto.getNombre(),
                        dto.getEmail(),
                        dto.getRol(),
    ...
    void obtenerUsuario_ok() {
        Mockito.when(usuarioService.obtenerPorId(1L)).thenReturn(
                new com.eventos.ms_usuarios.dto.UsuarioDto(1L, "Ana", "ana@example.com", RolUsuario.PROVEEDOR,
                        Instant.now(),
                        Instant.now()));
    ...
    @Test
    @DisplayName("GET /usuarios/{id} no existe -> 404")
    void obtenerUsuario_noExiste() {
        long id = 13L;
        Mockito.when(usuarioService.obtenerPorId(id))
                .thenThrow(new com.eventos.ms_usuarios.exception.RecursoNoEncontradoException("Usuario", id));

        webTestClient.get().uri("/usuarios/" + id)
                .accept(MediaType.APPLICATION_JSON)
                .exchange()
                .expectStatus().isNotFound()
                .expectBody()
                .jsonPath("$.message").isEqualTo("Usuario con id " + id + " no existe");
    }

    @Test
    @DisplayName("GET /usuarios/abc -> 400 parámetro inválido")
    void obtenerUsuario_parametroInvalido() {
        webTestClient.get().uri("/usuarios/abc")
                .accept(MediaType.APPLICATION_JSON)
                .exchange()
